...
| `startDate` / `endDate` | Inclusive date filters applied to each transaction's **authoritative** on-chain timestamp. Rows with an unknown timestamp are kept, never dropped. Filtering is integrated with pagination: because providers page newest-first, the walker stops as soon as it passes `startDate`, so it never silently misses older matches or loops unbounded. |
...
| `offset` | Rows requested per upstream page (EVM/TRON), capped at the provider's per-page maximum. |
| `maxPages` | Maximum number of upstream pages the walker will pull for this request. |

**Date filter semantics.** `startDate`/`endDate` accept either a bare
calendar date or a full ISO timestamp, and the boundary is resolved to make
the intuitive reading correct:

- `startDate=2026-09-01` → included from `2026-09-01T00:00:00.000Z` (start of the day).
- `endDate=2026-09-06` → included through `2026-09-06T23:59:59.999Z`, i.e. the **entire** day of Sep 6 is covered.
- A value that carries a time component (e.g. `2026-09-06T14:30:00Z`) is used **exactly**, with no day expansion.

Both bounds are inclusive. An unparseable date is ignored (treated as "no
bound") rather than excluding every row.
...
Investigation storage is selected solely by the `DATABASE_URL` environment
variable.

**No `DATABASE_URL`** — IN_MEMORY development mode (`DevelopmentDatabase`).
Investigations, transactions and reports are held in the in-memory store and
**do not survive a server restart**. This is the default and keeps the app
fully usable offline.

**`DATABASE_URL` set** — PostgreSQL persistent mode (`ProductionDatabase`).
Investigations and their related records (complaints, wallets, transactions,
alerts, evidence, risk assessments, VASP attributions, investigation events and
reports) are persisted and survive refresh, navigation and server restart. If
PostgreSQL is unreachable the app returns a clear error and never silently
falls back to in-memory storage.

Initialize (or migrate) the PostgreSQL schema — idempotent, safe to re-run:

```bash
pnpm db:init
```

`ProductionDatabase.init()` also applies the same idempotent DDL automatically
on first use, so running `pnpm db:init` is a convenience rather than a
requirement.
